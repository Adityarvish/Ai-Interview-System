...
// ── initSocket ───────────────────────────────────────────────────────────────
// FIX v15.1 — 400 Bad Request on GET /socket.io/socket.io.js
//
// The Socket.IO client requests /socket.io/socket.io.js during its HTTP
// long-polling handshake. python-socketio / FastAPI does not serve this
// static file, so the server returns 400.
//
// Setting transports: ['websocket'] and upgrade: false skips the polling
// handshake entirely. The client goes straight to a WebSocket connection
// and never makes the HTTP request that triggers the 400.
//
// The complementary HTML fix removes the document.write() fallback that
// was loading a duplicate Socket.IO client (doubling the 400s).
function initSocket() {
    socket = io(API, {
        path: '/socket.io',
        transports: ['websocket'],   // FIX: websocket-only; skips polling that causes 400
        upgrade: false,              // FIX: don't attempt polling upgrade
        reconnection: true,
