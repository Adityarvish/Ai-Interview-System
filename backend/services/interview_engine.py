...
class InterviewState:
    ...
    current_question_meta: Dict = field(default_factory=dict)
    ...
    def is_question_duplicate(self, question: str) -> bool:
        """ DuplicateDetector — checks ALL previous questions."""
...
class InterviewEngine:
    ...
    def initialize_interview(self, candidate_name: str, resume_path: str,
                             job_description: str, language: str = "en") -> dict:
    ...
    _GREETING_TEMPLATES = {
        "en": (
            "Hello {name}, welcome to the interview! "
            "I'm glad you could join us today. "
            "To begin, could you please briefly introduce yourself  "
            "walk me through your educational background, your key experience, "
            "and what drew you to apply for this role?"
        ),
        "hi": (
            "नमस्ते {name}, इंटरव्यू में आपका स्वागत है! "
            "आज आपके साथ बात करके खुशी हुई। "
            "शुरुआत के लिए, कृपया अपना संक्षिप्त परिचय दें  "
            "अपनी शिक्षा, मुख्य अनुभव, और इस भूमिका के लिए आवेदन करने का कारण बताएं?"
        ),
        "mr": (
            "नमस्कार {name}, मुलाखतीत आपले स्वागत आहे! "
            "आज तुमच्याशी बोलून आनंद झाला. "
            "सुरुवातीला, कृपया तुमची थोडक्यात ओळख द्या  "
            "तुमचे शिक्षण, मुख्य अनुभव आणि या भूमिकेसाठी अर्ज करण्याचे कारण सांगा?"
        ),
    }

    def generate_first_question(self) -> str:
        name = self.candidate_info['name']
        lang = self.candidate_info.get('language', 'en')
        template = self._GREETING_TEMPLATES.get(lang, self._GREETING_TEMPLATES['en'])
        greeting = template.format(name=name)
        self._record_question(greeting, stage="greeting")
        self.state.record_stage_question()
        logger.info(f"[GREETING] Greeting generated for '{name}' (lang={lang})")
        return greeting
    ...
    def generate_follow_up_question(self, previous_answer: str,
        ...
        if stage == "technical":
            ...
            for tech in all_tech:
                if (tech.lower() in question.lower() or
                    ...
                    if detected_topic == stage:   
                        detected_topic = tech
        elif stage == "behavioral":
            detected_topic = "Behavioral"
        ...
        self.state.current_question_meta = {
            "topic":         detected_topic,
            "difficulty":    "medium",
            "question_type": _STAGE_TO_TYPE.get(stage, "general"),
        }
    ...
    _CLOSING_TEMPLATES = {
        "en": (
            "Thank you so much, {name}  it's been a pleasure speaking with you today. "
            "You've given me a thorough picture of your background and technical skills. "
            "Our team will review your responses carefully and be in touch within the next few days."
        ),
        "hi": (
            "बहुत धन्यवाद, {name}  आज आपसे बात करके बहुत अच्छा लगा। "
            "आपने अपनी पृष्ठभूमि और तकनीकी कौशल की एक विस्तृत झलक दी है। "
            "हमारी टीम आपके उत्तरों की सावधानीपूर्वक समीक्षा करेगी और अगले कुछ दिनों में आपसे संपर्क करेगी।"
        ),
        "mr": (
            "खूप खूप धन्यवाद, {name}  आज तुमच्याशी बोलून खूप आनंद झाला. "
            "तुम्ही तुमची पार्श्वभूमी आणि तांत्रिक कौशल्ये यांची सविस्तर माहिती दिली. "
            "आमची टीम तुमच्या उत्तरांचे काळजीपूर्वक पुनरावलोकन करेल आणि येत्या काही दिवसांत तुमच्याशी संपर्क करेल."
        ),
    }
    ...
    def generate_closing_statement(self) -> str:
        ...
        lang = self.candidate_info.get('language', 'en')
        template = self._CLOSING_TEMPLATES.get(lang, self._CLOSING_TEMPLATES['en'])
        closing = template.format(name=name)
...
"""
    ...
    def _build_system_prompt(self):
        ...
        lang_code      = self.candidate_info.get('language', 'en')
        lang_name      = self._LANGUAGE_NAMES.get(lang_code, 'English')
        lang_line = (
            f"\nLANGUAGE: Write the question in {lang_name} "
            f"(native {lang_name} script, natural conversational tone, no English unless it's a technical term that has no {lang_name} equivalent)."
...
ROLE/JD: {jd}
TECH STACK: {jd_tech}
RESUME: {resume_summary}{lang_line}"""
    ...
    def _generate_and_validate(self, prompt: str, directive: str,
        ...
        for attempt in range(MAX_GENERATION_RETRIES):
            try:
                ...
                # Duplicate detected  add to rejected list and retry
