...
from services.per_question_background import (    
    trigger_per_question_eval,
    release_semaphore as release_pq_semaphore,
)
...
_PQ_EVAL_TASKS: dict[str, list[asyncio.Task]] = {}
...
def _cleanup_session(interview_id: str):
    ...
    _PQ_EVAL_TASKS.pop(interview_id, None)
    release_pq_semaphore(interview_id)          
...
def _tts_b64_blocking(text: str, interview_id: str, language: str = "en") -> tuple:
...
async def _tts_b64(text: str, interview_id: str, language: str = "en") -> tuple:
    loop = asyncio.get_event_loop()
    return await loop.run_in_executor(None, _tts_b64_blocking, text, interview_id, language)
...
async def _handle_audio_bytes(sid: str, interview_id: str, audio_bytes: bytes,
    ...
    async def _process():
        ...
        try:
            ...
            loop  = asyncio.get_event_loop()
            stt_language = (engine.candidate_info.get('language', 'en') if engine else 'en')
            try:
                transcript = await loop.run_in_executor(
                    None, stt_service.transcribe, transcribe_path, stt_language
                )
            except Exception as e:
            ...
            # Interaction logging removed — per_question_evaluations is the sole store.

            # ── Trigger background per-question evaluation ────────────────────
            
            if not _is_cancelled(interview_id) and current_q:
                q_meta        = engine.state.current_question_meta if hasattr(engine.state, 'current_question_meta') else {}
                q_topic       = q_meta.get('topic', engine.state.stage or 'General')
                q_difficulty  = q_meta.get('difficulty', 'medium')
                q_type        = q_meta.get('question_type', 'general')   # "technical" | "behavioral" | "general"

                task = asyncio.create_task(
                    trigger_per_question_eval(
                        interview_id    = interview_id,
                        question_id     = str(question_count),
                        question        = current_q,
                        answer          = transcript,
                ...
                _PQ_EVAL_TASKS.setdefault(interview_id, []).append(task)
                logger.info(
                    f"[PQ-EVAL] [{interview_id}] Per-question eval queued "
                    f"for Q{question_count} topic={q_topic} diff={q_difficulty}"
                )
            ...
            if is_final and not _is_cancelled(interview_id):
                ...
                tts_language = (engine.candidate_info.get('language', 'en') if engine else 'en')
                inline_audio_b64, inline_audio_fmt = await _tts_b64(ai_text, interview_id, tts_language)
            ...
            async def _deliver_audio():
                ...
                tts_language = (engine.candidate_info.get('language', 'en') if engine else 'en')
                audio_b64, audio_fmt = await _tts_b64(ai_text, interview_id, tts_language)
...
async def end_interview(sid, data):
    ...
    async def _finalize():
        ...
        try:
            ...
            # Wait for all background per-question eval tasks to finish
            # writing to DB before summarising — otherwise generate_interview_summary
            # may read 0 rows and produce a 0/100 result.
            pq_tasks = _PQ_EVAL_TASKS.get(interview_id, [])
            if pq_tasks:
                _pq_wait_start = time.perf_counter()
                logger.info(f"[END] [{interview_id}] Waiting for {len(pq_tasks)} per-question eval task(s)…")
                done, pending = await asyncio.wait(pq_tasks, timeout=60)
                for t in pending:
                    t.cancel()
                _pq_elapsed = int((time.perf_counter() - _pq_wait_start) * 1000)
                logger.info(
                    f"[END] [{interview_id}] Per-question evals done in {_pq_elapsed}ms "
                    f"({len(done)} done, {len(pending)} timed out)"
                )

            loop   = asyncio.get_event_loop()
            engine = ENGINES.get(interview_id)
            name = engine.candidate_info.get('name', 'Candidate') if engine else 'Candidate' 
            ...
            saved_doc = await loop.run_in_executor(
            ...
            _cleanup_session(interview_id)   # releases per-question eval semaphore
...
async def cancel_interview(sid, data):
    ...
    async def _cancel():
        ...
        try:
            ...
            # Wait for background per-question eval tasks before summarising
            pq_tasks = _PQ_EVAL_TASKS.get(interview_id, [])
            if pq_tasks:
                _pq_wait_start = time.perf_counter()
                logger.info(f"[CANCEL] [{interview_id}] Waiting for {len(pq_tasks)} per-question eval task(s)…")
                done, pending = await asyncio.wait(pq_tasks, timeout=60)
                for t in pending:
                    t.cancel()
                _pq_elapsed = int((time.perf_counter() - _pq_wait_start) * 1000)
                logger.info(
                    f"[CANCEL] [{interview_id}] Per-question evals done in {_pq_elapsed}ms "
                    f"({len(done)} done, {len(pending)} timed out)"
                )
            ...
            logger.info(f"[CANCEL] [{interview_id}] Done — summary generated")
            await _emit_to_room('evaluation_ready', {
                'interview_id': interview_id,
                'evaluation':   saved_doc,
            })
